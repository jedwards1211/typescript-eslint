...
TypeScript allows `+` adding together certain unsafe combinations of types.
Adding values that are not the same type and/or are not the same primitive type is often a sign of programmer error.
